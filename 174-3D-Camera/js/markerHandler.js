...
AFRAME.registerComponent("markerhandler", {
  ...
  handleRatings: async function(dish){
    ...
    var tNumber;
    table <=9 ? (tNumber = `T0${tableNumber}`): (tNumber = `T${tableNumber}`)
    //obtener el resumen de la orden desde la base de dates
    var orderSummary = await this.getOrderSummary(tNumber)
    var current_orders = Object.keys(orderSummary.curren.ordeers)
    if(current_orders.length > 0 && current_orders === dish.id){
      ...
      var rating = document.getElementById("rating-imput").value
      var feedback = document.getElementById("feedback-imput").value
      ...
      swal({
        ...
        text: "No hemos podidoencontrar ningun platillo",
        timer: 2500,
        button: false
      })
    }
